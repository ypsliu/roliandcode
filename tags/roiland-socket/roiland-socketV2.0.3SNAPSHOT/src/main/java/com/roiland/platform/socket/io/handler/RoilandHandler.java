...
public final class RoilandHandler extends SimpleChannelInboundHandler<ProtocolBean> {
            ...
            if (message instanceof ChainBean) {
            	...
            	if (len > 9) {
                	...
                	if (IS_OPEN_LOG && LOGGER.isInfoEnabled()) {
                    	ActorRef logger = this.context().actorOf(Props.create(LoggerActor.class));
                    	logger.tell(message, ActorRef.noSender());
					}
     ...
     * 日志处理Actor
     */
    public static class LoggerActor  extends UntypedActor {
        public void onReceive(Object message) {
            if (message instanceof ChainBean) {
            	final ChainBean chainBean = (ChainBean) message;
            	final Channel channel = chainBean.getChannel();
